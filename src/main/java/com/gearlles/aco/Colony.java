...
public class Colony {
	...
	private Arest chooseArest(Ant ant, Map map) {
		...
		for (int i = 0; i < possible.size(); i++) {
			...
			if (possible.contains(a)) {
				if (!ant.visitedArests.contains(a)) {
					sum += Math.pow(a.pherValue, a.pherInfluence) * Math.pow(heurs[i], a.visibilityInfluence);
				}
			}
		}
		...
		for (int i = 0; i < possible.size(); i++) {
			...
			choosed.probability = (Math.pow(choosed.pherValue, choosed.pherInfluence) * Math.pow(heurs[i],
					choosed.visibilityInfluence)) / sum;
		}

		// for (Arest choosed : possible)
		// {
		// sum += choosed.pherInfluence * choosed.pherValue * (1.00 /
		// choosed.length) * choosed.visibilityInfluence;
		// }
		//
		// for (Arest choosed : possible)
		// {
		// choosed.probability = Math.random() * ((choosed.pherInfluence *
		// choosed.pherValue * (1.00 / choosed.length) *
		// choosed.visibilityInfluence)/sum);
		// }
	...
	private ArrayList<Arest> possibleArests(Ant ant, Map map) {
		...
		for (int i = 0; i < map.paths.size(); i++) {
			if (map.paths.get(i).source.id == ant.actualNode.id || map.paths.get(i).destination.id == ant.actualNode.id) {
				if ((!ant.visitedNode.contains(map.paths.get(i).source) || !ant.visitedNode
						.contains(map.paths.get(i).destination))) {
					possible.add(map.paths.get(i));
				} else if (!ant.visitedArests.contains(map.paths.get(i))
						&& (map.paths.get(i).source == ant.source || map.paths.get(i).destination == ant.source)
						&& allNodesVisited(ant, map) && ant.actualNode != ant.source) {
					possible.add(map.paths.get(i));
				}
			}
		}
